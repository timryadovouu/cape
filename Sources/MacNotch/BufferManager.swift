...
final class BufferManager: ObservableObject {
    ...
    /// Pasteboard marker types that clipboard managers should never store — the
    /// nspasteboard.org convention. Password managers stamp copied passwords with
    /// `ConcealedType`, so this keeps passwords out of the buffer entirely.
    private static let skipPasteboardTypes: Set<String> = [
        "org.nspasteboard.ConcealedType",      // passwords (1Password, Bitwarden, …)
        "org.nspasteboard.TransientType",      // clips explicitly marked "don't store"
        "org.nspasteboard.AutoGeneratedType",  // machine-generated clips
    ]
    ...
    private func check() {
        ...
        // Never capture passwords or clips flagged "don't store" (nspasteboard.org).
        if let types = pb.types, types.contains(where: { Self.skipPasteboardTypes.contains($0.rawValue) }) {
            return
        }
